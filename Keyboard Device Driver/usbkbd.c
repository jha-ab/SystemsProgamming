...
static int usb_kbd_event(struct input_dev *dev, unsigned int type,unsigned int code, int value){
	...
	change_led = (kbd->newleds) & 0x03;
	printk(" change LED is %d leds is %d", change_led , *(kbd->leds));

   if (kbd->mode == 1)
	{
		if(change_led == 0x01 )
		{
			printk("Changing to mode 2\n");
			kbd->newleds = 0x03;
			kbd->mode = 2;
		}
		if(change_led == 0x03 )
		{
			printk("Do Not change mode 2\n");
			kbd->newleds = 0x02;
		}
	}
	else if(kbd->mode == 2 )
	{
     if(kbd->newleds == 0x00) 
	 {
        printk( "Changing to mode 1");
        kbd->newleds = 0x00; 
        kbd->mode = 1;
     } 
	 else if(kbd->newleds == 0x03 )
	  {
        kbd->newleds = 0x01;
     } 
	else if(kbd->newleds == 0x01)
	{
        kbd->newleds = 0x03;
    } 
	else if(kbd->newleds == 0x02) 
	{
        printk( "Changing to mode 1");
        kbd->newleds = 0x02;
        kbd->mode = 1;
	...
	printk(" NEW : change LED is %d leds is %d", kbd->newleds , *(kbd->leds));
...
static int usb_kbd_probe(struct usb_interface *iface,			 const struct usb_device_id *id){
	...
	printk("Kernel Alert : Probe USB called \n");
...
static void usb_kbd_disconnect(struct usb_interface *intf){
	...
	printk("Kernel Alert :KBD Disconnected \n");
